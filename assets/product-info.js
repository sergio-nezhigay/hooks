if (!customElements.get("product-info")) {
  customElements.define(
    ...
    class ProductInfo extends HTMLElement {
      ...
      updateDiscountTable(id, url) {
        function getVariantDetails() {
          const selectedOptionId = id;
          // Fetch Shopify product JSON (ensure it's available)
          fetch(window.location.pathname + ".js")
            .then((response) => response.json())
            .then((product) => {
              const variant = product.variants.find((v) => {
                if (v.title == selectedOptionId) {
                  return v;
                }
              });
              if (variant) {
                const vprice = variant.price / 100;
                const off5 = document.querySelector("[data-price-5-off]");
                const off10 = document.querySelector("[data-price-10-off]");
                const off15 = document.querySelector("[data-price-15-off]");
                const off20 = document.querySelector("[data-price-20-off]");
                if (off5 && off10 && off15 && off20) {
                  off5.innerHTML = "$" + (vprice * 0.95).toFixed(2);
                  off10.innerHTML = "$" + (vprice * 0.9).toFixed(2);
                  off15.innerHTML = "$" + (vprice * 0.85).toFixed(2);
                  off20.innerHTML = "$" + (vprice * 0.8).toFixed(2);
                }
              } else {
      ...
      handleOptionValueChange({
        ...
        this.updateDiscountTable(target.getAttribute("value"), productUrl);
